        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 card-hover">
            <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                    ...
                    {/* Display criteria for wants */}
                    {type === 'want' && item.criteria && item.criteria.length > 0 && (
                        <div className="mt-3">
                            <p className="text-xs font-medium text-slate-500 mb-2">Requirements:</p>
                            <div className="flex flex-wrap gap-1">
                                {item.criteria.map((criterion: string) => (
                                    <span 
                                        key={criterion} 
                                        className="px-2 py-1 text-xs bg-indigo-100 text-indigo-700 rounded-full font-medium"
                                    >
                                        {CRITERIA_LABELS[criterion as keyof typeof CRITERIA_LABELS] || criterion}
                                    </span>
                                ))}
                            </div>
                        </div>
